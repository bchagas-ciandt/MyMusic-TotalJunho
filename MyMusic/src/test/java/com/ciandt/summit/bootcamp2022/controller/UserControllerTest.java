...
public class UserControllerTest {
    ...
    void findUserByIdShouldReturnStatusOk() throws Exception{
        ...
        ResultActions status = this.mockMvc.perform(get("http://localhost:8080/api/v1/users/userId"))
                .andExpect(status().isOk());

        Assertions.assertEquals(200,status.andReturn().getResponse().getStatus());
